...
* Sie können das Jave Executor Framework zum parallelen Ausführen von nebenläufigen Jobs praktisch anwenden.


=== Voraussetzungen
* Vorlesung Concurrency - Execution 1 & 2
...
Im {handout}[Praktikumsverzeichnis] finden sie das Modul `PrimeChecker`.
...
* **`MandelbrotCallableProcessor`**: Hier wiederum ein `ExecutorService` verwendet werden. Diesmal aber soll der `MandelbrotTask` als Callable umgesetzt werden, der jeweils eine Zeile als `Future<ImageRow>` zurückgibt.
...
Sie müssen nur diese Klassen bearbeiten und anpassen. Die Benutzeroberfläche und Hilfsklassen sind im übergeordneten
...
inneren Klassen `MandelbrotTask`, so dass sie die obige Beschreibung erfüllen.
